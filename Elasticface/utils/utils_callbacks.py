...
from eval import verification as verification
...
class CallBackVerification(object):
    ...
    def ver_test(self, backbone: torch.nn.Module, global_step: int):
        ...
        for i in range(len(self.ver_list)):
            acc1, std1, acc2, std2, xnorm, embeddings_list,prec,rec= verification.test(
            ...
            results.append(acc2)
            logging.info('[%s][%d]precision : %f' % (self.ver_name_list[i], global_step, prec))
            logging.info('[%s][%d]recall: %f' % (self.ver_name_list[i], global_step, rec))
